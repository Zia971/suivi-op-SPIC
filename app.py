...
def page_manager():
    ...
    with col3:
        ops_en_cours = len(df_operations[df_operations['statut_principal'].str.contains('travaux|consultation')])
        st.metric("En Cours", ops_en_cours, delta=None)
    
    with col4:
        ops_bloquees = len(df_operations[df_operations['statut_principal'].str.contains('Bloqué')])
    ...
    df_risque['score_risque'] = (
        ...
        df_risque['statut_principal'].str.contains('Bloqué|retard').astype(int) * 30
    )
...
def render_phases_tab(operation_id):
    ...
    for phase in phases:
        ...
        with col5:
            ...
            couleur = "🟢" if est_validee else "🔴" if datetime.now().date() > date_fin if date_fin else False else "🟡"
            st.write(couleur)
